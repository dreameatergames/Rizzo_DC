...
cvar_t	r_wateralpha = {"r_wateralpha","0.6", true}; // Manoel Kasimier - translucent water
...
===============
*/
//void SCR_Adjust (void); // Manoel Kasimier
...
//		cl_visedicts[i]->effects -= cl_visedicts[i]->effects & (EF_CELSHADING|EF_REFLECTIVE); // for betatesting
		// Manoel Kasimier - begin
		if (cl_visedicts[i]->alpha != 1.0f				// Manoel Kasimier - QC Alpha
...
// clamp lighting so it doesn't overbright as much
	...
	*/ // Manoel Kasimier - changed alias models lighting - removed - end
	...
	if (r_dspeeds.value) // Manoel Kasimier
		dv_time1 = Sys_FloatTime (); // Manoel Kasimier - draw viewmodel time
	// Manoel Kasimier - EF_CELSHADING - begin
	...
	if (r_dspeeds.value) // Manoel Kasimier
		dv_time2 = Sys_FloatTime (); // Manoel Kasimier - draw viewmodel time
}
...
//	byte	warpbuffer[WARP_WIDTH * WARP_HEIGHT]; // Manoel Kasimier - hi-res waterwarp & buffered video - removed
	...
	if (warpbuffer)
		free(warpbuffer);
	warpbuffer = malloc(vid.rowbytes*vid.height);
...
//		de_time1 = se_time2; // draw entities time
	...
	// Manoel Kasimier - QC Alpha - end
	if (r_dspeeds.value) // Manoel Kasimier
		de_time1 = Sys_FloatTime (); // Manoel Kasimier - draw entities time
