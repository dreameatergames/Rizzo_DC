...
void Draw_TransPic (int x, int y, qpic_t *pic)
{
	...
	// mankrip - begin
	// workaround for Sys_Error ("Draw_TransPic: bad coordinates");
	x = (x < 0) ? 0 : ( ( (unsigned)(x + pic->width ) > vid.width ) ? x - pic->width  : x);
	y = (y < 0) ? 0 : ( ( (unsigned)(y + pic->height) > vid.height) ? y - pic->height : y);
	// mankrip - end

	if (x < 0 || (unsigned)(x + pic->width) > vid.width ||
		y < 0 || (unsigned)(y + pic->height) > vid.height)
		Sys_Error (va("Draw_TransPic: bad coordinates (%i, %i)", x, y)); // mankrip
...
//	unsigned short	*pusdest;
	...
	if (x < 0 || (unsigned)(x + pic->width) > vid.width ||
		y < 0 || (unsigned)(y + pic->height) > vid.height)
		Sys_Error (va("Draw_TransPicMirror: bad coordinates (%i, %i)", x, y)); // mankrip
...
void Draw_TransPicTranslate (int x, int y, qpic_t *pic, byte *translation)
{
	...
	{
		Sys_Error (va("Draw_TransPicTranslate: bad coordinates (%i, %i)", x, y)); // mankrip
	}
	...
	if (r_pixbytes == 1)
	{
		...
		if (con_alpha.value < 0.5 && !con_forcedup)
			...
			for (y=0 ; y<lines ; y++, dest += vid.conrowbytes)
			{
				...
				for (x=0 ; x<vid.conwidth ; x+=4)
				{
					dest[x] = alphamap[dest[x] + src[f>>16]*256];
					f += fstep;
					dest[x+1] = alphamap[dest[x+1] + src[f>>16]*256];
					f += fstep;
					dest[x+2] = alphamap[dest[x+2] + src[f>>16]*256];
					f += fstep;
					dest[x+3] = alphamap[dest[x+3] + src[f>>16]*256];
					f += fstep;
				}
			}
		else
