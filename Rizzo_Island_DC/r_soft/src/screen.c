...
/*	// Manoel Kasimier - r_letterbox - start
	{
...
*/	// Manoel Kasimier - r_letterbox - end
}
/*
==================
SCR_Init
...
void SCR_DrawRam (void)
{
	...
	Draw_TransPic (scr_vrect.x+32, scr_vrect.y, scr_ram); // Manoel Kasimier - edited
}
...
void SCR_DrawFPS (void)
{
	...
	double t;
	static int lastfps;
	int x;//, y; // Manoel Kasimier - edited
	char st[80];

	t = Sys_FloatTime ();
	if ((t - lastframetime) >= 0.5) {
		lastfps = fps_count;
		fps_count = 0;
		lastframetime = t;
	}
	...
	x = vid.width - strlen(st) * 8 - screen_right; // Manoel Kasimier - edited
	if (scr_vrect.y > screen_top) // Manoel Kasimier
		Draw_Fill(x, screen_top, strlen(st) * 8, 8, 0); // Manoel Kasimier
...
{
	extern cvar_t sbar; // Manoel Kasimier - Fightoon addition
	...
	if (sbar.value != 5) // Manoel Kasimier - Fightoon addition
