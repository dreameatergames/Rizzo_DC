...
static int		vid_stretched;//, windowed_mouse; // Manoel Kasimier - removed
...
//cvar_t		_windowed_mouse = {"_windowed_mouse","0", true}; // Manoel Kasimier - removed
...
void VID_InitMGLDIB (HINSTANCE hInstance)
{
    ...
    wc.lpszMenuName  = 0;
    wc.lpszClassName = "Makaqu"; // WinQuake // Manoel Kasimier - edited
...
// for the rest of the session
	if (!vid_mode_set)
	{
		mainwindow = CreateWindowEx (
			 ExWindowStyle,
			 "Makaqu", // WinQuake // Manoel Kasimier - edited
			 "Makaqu", // WinQuake // Manoel Kasimier - edited
			 WindowStyle,
			 0, 0,
...
// empty while Quake starts up.
	...
	vid.maxwarpheight = vid.height; // Manoel Kasimier - hi-res waterwarp
...
//	if (r_dowarp) // Manoel Kasimier - buffered video (bloody hack) - removed
		d_viewbuffer = r_warpbuffer;
//	else // Manoel Kasimier - buffered video (bloody hack) - removed
//		d_viewbuffer = (void *)(byte *)vid.buffer; // Manoel Kasimier - buffered video (bloody hack) - removed

//	if (r_dowarp) // Manoel Kasimier - buffered video (bloody hack) - removed
		screenwidth = vid.width;//WARP_WIDTH; // Manoel Kasimier - hi-res waterwarp - edited
//	else // Manoel Kasimier - buffered video (bloody hack) - removed
//		screenwidth = vid.rowbytes; // Manoel Kasimier - buffered video (bloody hack) - removed
...
//	Cvar_RegisterVariable (&_windowed_mouse); // Manoel Kasimier - removed
...
// minimize/restore fulldib windows/mouse-capture normal windows on demand
	if (!in_mode_set)
	{
		if (ActiveApp)
		{
			...
			else if ((modestate == MS_WINDOWED) && key_dest != key_menu && !cl.paused) // Manoel Kasimier - edited
			{
				IN_ActivateMouse ();
				IN_HideMouse ();
			}
...
void VID_HandlePause (qboolean pause)
{
	...
	if ((modestate == MS_WINDOWED)) // Manoel Kasimier - edited
	if (key_dest != key_menu) // Manoel Kasimier
	{
		if (pause)
		{
			IN_DeactivateMouse ();
...
extern void M_Video_f (void); // Manoel Kasimier - edited
...
void VID_MenuDraw (void)
{
	...
	M_DrawTransPic ( (320-p->width)/2, 4, p); // Manoel Kasimier - edited
